...
def build_manifest(config: ProtocolConfig) -> pd.DataFrame:
    ...
    if config.sampling_mode == "per_class":
        for class_position, synset_dir in enumerate(discovery.synset_dirs):
            ...
            sampled = [
                image_files[int(pos)]
                for pos in shuffled_positions[: config.main_images_per_class + config.control_images_per_class]
            ]
    ...
    elif config.sampling_mode == "global_arbitrary":
        all_images: list[tuple[int, str, Path]] = []
        for class_position, synset_dir in enumerate(discovery.synset_dirs):
            synset = synset_dir.name
            if synset not in synset_to_idx:
                raise KeyError(f"Missing ImageNet idx mapping for synset {synset}")
            for image_path in _sorted_image_files(synset_dir):
                all_images.append((class_position, synset, image_path))

        total_needed = config.main_images_target + config.control_images_target
        if len(all_images) < total_needed:
            raise ValueError(f"Only found {len(all_images)} images but need {total_needed}.")

        sampled_positions = rng.permutation(len(all_images))[:total_needed]
        sampled = [all_images[int(pos)] for pos in sampled_positions]
        main_samples = sampled[: config.main_images_target]
        control_samples = sampled[config.main_images_target :]

        train_cutoff = int(round(config.main_images_target * config.probe_train_ratio))
        train_cutoff = min(max(train_cutoff, 0), config.main_images_target)
        if config.main_images_target > 1:
            train_cutoff = min(max(train_cutoff, 1), config.main_images_target - 1)

        for local_position, (class_position, synset, image_path) in enumerate(main_samples):
            probe_split = "train" if local_position < train_cutoff else "test"
            rows.append(
                {
                    "image_id": f"global_main_{local_position:04d}",
                    "relative_path": str(image_path.relative_to(discovery.root)),
                    "absolute_path": str(image_path),
                    "synset": synset,
                    "imagenet_idx": synset_to_idx[synset],
        ...
        for local_position, (class_position, synset, image_path) in enumerate(control_samples):
            rows.append(
                {
                    "image_id": f"global_control_{local_position:04d}",
                    "relative_path": str(image_path.relative_to(discovery.root)),
                    "absolute_path": str(image_path),
                    "synset": synset,
                    "imagenet_idx": synset_to_idx[synset],
    ...
    main_count = sum(1 for row in rows if row["subset_role"] == "main")
    if config.preview_images > main_count:
        raise ValueError(f"preview_images={config.preview_images} exceeds main image count {main_count}.")
