...
def block_mixing(context, x, context_block, x_block, c, return_act=None):
    ...
    if return_act:
        x_q, x_k, x_v = x_qkv
        b__, _, dim_head__ = x_q.shape
        num_heads__ = x_block.attn.num_heads
        dim_head__ //= num_heads__
        x_q, x_k, x_v = map(lambda t: t.view(b__, -1, num_heads__, dim_head__).transpose(1, 2), (x_q, x_k, x_v))
        scale__ = dim_head__ ** -0.5
        attn_logits = (x_q @ x_k.transpose(-2, -1)) * scale__
...
class MMDiT(nn.Module):
    ...
    def forward_core_with_concat(
            ...
            detach: Optional[bool] = False, return_act=None) -> torch.Tensor:
        if self.register_length > 0:
        ...
        for i, block in enumerate(self.joint_blocks):
            ...
            if ((i+1) <= self.encoder_depth) and (return_act is not None):
                context, x, attn_logits = block(context, x, c=c_mod, return_act=True)
                attn_.append(attn_logits)
    ...
    def forward(
        ...
        if return_act:
            x, attn_, zs = self.forward_core_with_concat(x, c, context, detach, return_act=True)
        else:
